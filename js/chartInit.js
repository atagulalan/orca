...
function drawIntersection(runIndex, tId, MFS, ordersChartData, chartInstance) {
  ...
  Object.keys(MFS).forEach(function(k, i) {
    ...
    zeroPointY =
      Y.top + ((Y.bottom - Y.top) / (Y.ticks.length - 1)) * Y.zeroLineIndex;
    zeroPointX = Y.right;
  ...
  if (Object.keys(zart[tId]).length > 0) {
    let s = Object.keys(zart[tId]).reduce((a, b) =>
      zart[tId][a] > zart[tId][b] ? a : b
    );
    console.log(tId, s);
    textify(tId, s);
  }
